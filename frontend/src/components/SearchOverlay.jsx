  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          {/* Header */}
          <div className="flex items-center gap-4 mb-10 max-w-5xl mx-auto w-full">
            <div className="flex-1 relative group">
              <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-brand-primary" size={28} />
              <input
                autoFocus
                type="text"
                value={localQuery}
                ...
                className="w-full pl-16 pr-6 py-7 bg-bg-secondary rounded-[3rem] border-2 border-brand-primary/20 focus:border-brand-primary outline-none text-2xl font-black italic text-text-primary shadow-2xl transition-all"
              />
            </div>
            <button
              onClick={onClose}
              className="p-6 bg-bg-secondary rounded-full text-text-secondary hover:text-red-500 transition-all border border-border-primary shadow-lg"
            >
              <X size={32} />
            </button>
          </div>

          <div className="flex-1 overflow-y-auto no-scrollbar max-w-5xl mx-auto w-full space-y-12 pb-20">
            {debouncedQuery ? (
                ...
                {searchResults.shops.length === 0 && searchResults.products.length === 0 && (
                  <div className="text-center py-20 flex flex-col items-center gap-6">
                    <div className="w-20 h-20 bg-bg-secondary rounded-full flex items-center justify-center border border-border-primary">
                      <Search size={40} className="text-text-secondary opacity-20" />
                    </div>
                    <div>
                      <h4 className="text-xl font-bold text-text-primary mb-2 italic">NO DECRYPTIONS FOUND</h4>
                      <p className="text-sm text-text-secondary font-medium opacity-60 max-w-xs mx-auto mb-8">
                        We couldn't find any shops or spares matching "{debouncedQuery}".
                      </p>
                      <button
                        onClick={() => navigate('/customer/home')}
                        className="px-8 py-3 bg-brand-primary/10 text-brand-primary rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-brand-primary hover:text-white transition-all glow-effect"
                      >
                        VIEW ALL SHOPS
                      </button>
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {trending.map((item, i) => (
                      <button
                        key={i}
                        onClick={() => handleSearch(item.name)}
                        className="flex justify-between items-center p-6 bg-bg-secondary rounded-3xl border border-border-primary hover:border-brand-primary/30 transition-all group glow-effect"
                      >
                        <div className="flex items-center gap-4">
                          <div className="p-3 bg-brand-primary/10 rounded-2xl text-brand-primary group-hover:bg-brand-primary group-hover:text-white transition-colors">{item.icon}</div>
                          <span className="font-black text-xs uppercase tracking-widest text-text-primary">{item.name}</span>
                        </div>
                        <ArrowRight size={16} className="text-text-secondary group-hover:text-brand-primary transition-colors" />
                      </button>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    {recent.map((q, i) => (
                      <button
                        ...
                        className="w-full flex items-center justify-between p-6 bg-bg-primary border border-border-primary rounded-[2.5rem] hover:bg-bg-secondary transition-all group"
                      >
                        <span className="font-black text-sm italic text-text-primary group-hover:text-brand-primary transition-colors">{q}</span>
            ...
            <div className="pt-8 border-t border-border-primary/50">
              <button
                ...
                className="w-full py-7 bg-gradient-to-r from-brand-primary to-brand-secondary text-white rounded-[2.5rem] font-black italic text-xl shadow-2xl shadow-brand-primary/20 flex items-center justify-center gap-3 glow-effect"
              >
                EXECUTE SEARCH <Zap size={24} strokeWidth={3} />
              </button>
            </div>
          </div>
        </motion.div>
